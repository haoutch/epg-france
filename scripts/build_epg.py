...
def download_source(dest):
    req = urllib.request.Request(
        SOURCE_URL,
        headers={"User-Agent": "Mozilla/5.0 EPG builder"},
    )
...
def choose_source(row, by_id, by_name, aliases):
    ...
    if scored and (
        len(scored) == 1
        or scored[0][0] - scored[1][0] >= MIN_MARGIN
    ):
...
def shift_ts(ts):
    ...
    dt += timedelta(hours=TIME_SHIFT_HOURS)
...
def main():
    ...
    wanted = {
        r["tvg_id_final"]: r
        for r in rows
        if r.get("tvg_id_final")
    }

    if not wanted:
        raise SystemExit(
            "Aucun tvg_id_final exploitable dans mapping_chaines.csv"
        )
    ...
    with report.open("w", encoding="utf-8", newline="") as f:
        ...
        for tid, row in wanted.items():
            w.writerow([
                ...
                row["nom_chaine"],
                resolved.get(tid, ""),
                methods.get(tid, "unmapped"),
            ])

    out_path = OUT / "epg_france.xml"
            ...
            if elem.tag == "channel":
                ...
                if dst:
                    ...
                    if "start" in elem.attrib:
                        elem.set("start", shift_ts(elem.get("start")))
    ...
    gz_output = OUT / "epg_france.xml.gz"

    with open(out_path, "rb") as f_in, gzip.open(
        gz_output,
        "wb",
        compresslevel=9,
    ) as f_out:
    ...
    status = {
        ...
        "matching_methods": {
            m: list(methods.values()).count(m)
            for m in sorted(set(methods.values()))
        },
        "generated_at_utc": (
    ...
    if not status["ok"]:
        raise SystemExit(
            "EPG invalide: aucun programme exploitable."
        )
...
def choose_source(row, by_id, by_name, aliases):
    ...
    # Correspondance directe par tvg-id
    if tid and norm_id(tid) in by_id:
        return by_id[norm_id(tid)][0], 'original'
...
def shift_ts(ts):
    ...
    dt = (
        ...
        + timedelta(hours=TIME_SHIFT_HOURS)
    )
...
def main():
    ...
    # Téléchargement de la source EPG
    gz_path = ROOT / 'epg_source.xml.gz'
            ...
            if elem.tag == 'channel':
                ...
                if dst:
                    ...
                    if 'start' in elem.attrib:
                        elem.set(
                            'start',
                            shift_ts(elem.get('start'))
                        )

                    if 'stop' in elem.attrib:
                        elem.set(
                            'stop',
                            shift_ts(elem.get('stop'))
                        )
    ...
    # Création du fichier GZIP publié
    gz_output = OUT / 'epg_france.xml.gz'
    ...
    # Status publié sur GitHub Pages
    (
        OUT / 'status.json'
    ).write_text(
        json.dumps(
